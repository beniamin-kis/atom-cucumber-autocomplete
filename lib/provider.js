...
const PATH_CONFIG_KEY = 'cucumber-autocomplete.features_path';
const STEPS_CONFIG_KEY = 'cucumber-autocomplete.steps_path';
const CUCUMBER_STEP_DEF_PATTERN = /(Given|And|When|Then|But)\(\/\^(.*?)\$/g;
const CUCUMBER_KEYWORDS_PATTERN = /(Given|And|When|Then|But)(.*)/g;
const PROPERTY_PREFIX_PATTERN = /(?:^|\[|\(|,|=|:|\s)\s*((?:And|Given|Then|When|But)\s(?:[a-zA-Z]+\.?){0,2})$/;
...
module.exports = {
  ...
  getCompletions: function(line, file) {
    ...
    try {
      let stats = fs.lstatSync(`${this.rootDirectory()}${this.stepsDirectory()}`)
  ...
  scanStepDefinitionsDir: function(results = []) {
    ...
    const searchPath = `${this.rootDirectory()}${this.stepsDirectory()}`;
  ...
  stepsDirectory: function(path=STEPS_CONFIG_KEY) {
    return atom.config.get(path);
  },
